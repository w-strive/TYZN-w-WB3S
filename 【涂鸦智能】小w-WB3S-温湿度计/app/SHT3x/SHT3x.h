...
#define SHT3x_Write_Addr 0x88


//-- Enumerations --------------------------------------------------------------
// Sensor Commands
typedef enum{
	CMD_READ_SERIALNBR  = 0x3780, // 读取序列号
	CMD_READ_STATUS     = 0xF32D, // 读状态寄存器
	CMD_CLEAR_STATUS    = 0x3041, // 明确的状态寄存器
  ...
  CMD_NO_SLEEP        = 0x303E,
	CMD_W_Break_Stop    = 0x3093, // 停止周期测量
}etCommands;
...
//                      NO_ERROR       = no error
//------------------------------------------------------------------------------
u8 SHT3X_WriteCommand(etCommands cmd);
